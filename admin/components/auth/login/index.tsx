...
const LoginPage = () => {
  ...
  const { setAccessToken, setRefreshToken, setUser, user, refreshToken, accessToken } = useAuth();
  ...
  return (
    <div className="px-8 h-screen grid grid-cols-2 gap-4">
      <LoginIllustration />
      <FormPage formData={formData} handleOnChange={handleOnChange} handler={handleLogin} />
    </div>
  );
};
